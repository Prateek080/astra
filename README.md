...
|---|---|---|
| **All projects** | Add `alias claude='claude --plugin-dir ~/astra'` to `~/.zshrc`, then `source ~/.zshrc` | You want Astra in every session automatically |
| **Single project** | `claude --plugin-dir ~/astra` | You want Astra only for the current session |

**Verify:** Run `/help` inside Claude Code — you should see all `/astra:*` commands.

> **For teams:** Each developer clones Astra and picks their preferred method above. The project-specific files generated by `/astra:init` (`CLAUDE.md`, `.claude/rules/`) get committed to git — that's how the team shares project context.
